...
const UserBar = ({ onProfile, onLogout, className = "", isHome = true }) => {
  ...
  return (
    <div className={clsx(styles.userBar, className)} ref={ref}>
      <button type="button" onClick={() => setOpen((v) => !v)} className={styles.userBarBtn}>
        <AvatarIcon
          src={user.avatar}
          name={user.name}
          alt="avatar"
      ...
      {open && (
        <div className={clsx(styles.dropdown, !isHome && styles.dropdownDark)}>
          <button
            type="button"
            className={clsx(styles.dropdownItem, !isHome && styles.dropdownItemDark)}
            onClick={onProfile}
          >
            PROFILE
          </button>
          ...
          >
            LOG OUT
            <svg
              className={clsx(styles.arrowIcon, !isHome && styles.arrowIconDark)}
              width="18"
              height="18"
            >
